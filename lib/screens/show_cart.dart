...
class _ShowCartState extends State<ShowCart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('จองคิวของฉัน'),
      ),
      body: status
          ? Center(
              child: Text('งานรังวัดว่างเปล่า'),
            )
          : buildContent(),
    );
  }
        ...
        children: <Widget>[
          Expanded(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.end,
              children: <Widget>[
                MyStyle().showTitleH2('ประมาณการ : '),
              ],
            ),
          ),
          Expanded(
            flex: 1,
            child: MyStyle().showTitleH3Red(' บาท'),
          )
        ],
      );
    ...
    return Container(
      ...
      child: Column(
        children: <Widget>[
          Row(
            children: <Widget>[
              MyStyle().showTitleH2('สำนักงาน ${cartModels[0].nameShop}'),
            ],
          ),
          Row(
            children: <Widget>[
          ...
          Row(
            children: <Widget>[
              MyStyle()
                  .showTitleH3('ค่าเดินทาง = ${cartModels[0].transport} บาท'),
            ],
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          Expanded(
            flex: 3,
            child: MyStyle().showTitleH3('ประเภทงาน'),
          ),
          Expanded(
            flex: 1,
            child: MyStyle().showTitleH3('ปฎิบัติงาน'),
          ),
          Expanded(
            flex: 1,
            child: MyStyle().showTitleH3('ไร่'),
          ),
          Expanded(
            flex: 1,
            child: MyStyle().showTitleH3('รายละเอียด'),
          ),
          Expanded(
            flex: 1,
  ...
  Future<Null> confirmDeleteAllData() async {
    showDialog(
      ...
      builder: (context) => SimpleDialog(
        title: Text('คุณต้องการจะลบ รายการจองคิว จริงๆ หรือ ?'),
        children: <Widget>[
          Row(
  ...
  Future<Null> orderThread() async {
    ...
    await Dio().get(url).then((value) {
      if (value.toString() == 'true') {
        ...
        normalDialog(context, 'ไม่สามารถ จองคิว ได้ กรุณาลองใหม่');
      }
    });
  }
  ...
  Future<Null> notificationToShop(String idShop) async {
    ...
    await Dio().get(urlFindToken).then((value) {
      ...
      for (var json in result) {
        ...
        String title = 'มี รายการจองคิว';
        String body = 'รายการงานสำรวจรังวัด';
        String urlSendToken =
  ...
  Future<Null> sendNotificationToShop(String urlSendToken) async {
    await Dio().get(urlSendToken).then(
          (value) => normalDialog(context, 'ส่ง รายการจองคิว ไปที่ ส.ป.ก.จังหวัด แล้ว'),
        );
  }
}
